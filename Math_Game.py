...
class MathGame:
    ...
    def show_choices(self):
        ...
        while len(choices) < 4:
            if self.game_mode == 'Sub':
                choice = random.randint(-70,int(self.answer))
            elif self.game_mode == 'Mix':
                choice = random.randint(-100,int(self.answer))
            else : 
                choice = random.randint(1,int(self.answer))
    ...
    def activate_timer(self):
        ...
        if self.timer > 0 :
            ...
            self.root.after(1000,self.activate_timer)
